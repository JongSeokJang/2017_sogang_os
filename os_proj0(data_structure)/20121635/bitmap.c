...
struct bitmap *bitmap_expand(struct bitmap *a, int size)
{
	int ii = 0;
	size_t old_size = bitmap_size(a);
	size_t new_size = old_size + (size_t)size;
	struct bitmap *new_bitmap =  bitmap_create(new_size);

	// copy old bitmap setting to new bitmap
	for( ii = 0; ii < old_size; ii++ ){
		if( bitmap_test(a, ii) == 1 )
			bitmap_set(new_bitmap, ii, 1);
	}
